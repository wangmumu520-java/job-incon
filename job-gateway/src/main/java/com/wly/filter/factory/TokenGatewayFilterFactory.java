package com.wly.filter.factory;

import com.wly.constant.RedisCst;
import com.wly.exceptions.IllegalTokenException;
...
public class TokenGatewayFilterFactory extends AbstractGatewayFilterFactory<TokenGatewayFilterFactory.Config> {
    ...
    @Override
    public GatewayFilter apply(Config config) {//基于当前类中Config创建对象
        return new GatewayFilter() {    // servlet service httpServletRequest  httpServletResponse 传统web springmvc   springwebflux new web模型 //filter   request response filterChain.dofilter(request,response)
            ...
            public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
                ...
                if (config.requiredToken) {
                    ...
                    //2.根据token信息去redis获取
                    if (!redisTemplate.hasKey(RedisCst.TOKEN_KEY + token))
